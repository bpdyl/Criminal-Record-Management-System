...
<div class="form-group">
        ...
        <option  value="<?php echo htmlentities($crime['criminal_id']);?>"><?php echo htmlentities($criName=$crime['name']);?></option>
        <?php 
         $sql1 = "select * from tbl_criminal";
                $res1 = $connection->query($sql1);
                $data1 = $res1->fetch_assoc();
                if($res1->num_rows >0){
                    foreach ($data1 as $key => $criminal) {
    ...
    <div class="form-group">
        ...
        <select class="form-control" name="category" required="required">
         <option  value="<?php echo htmlentities($crime['category_id']);?>"><?php echo htmlentities($catName=$crime['category_name']);?></option>
        <?php 
         $sql2 = "select * from tbl_crime_category";
                $res2 = $connection->query($sql2);
                $data2 = $res2->fetch_assoc();
                if($res2->num_rows >0){
                    foreach ($data2 as $key => $category) {
    ...
    <div class="form-group">
        ...
        <select class="form-control" name="prison" required="required">
            <option value="<?php echo htmlentities($crime['prison_id']); ?>"><?php echo htmlentities($priName=$crime['prison_name']); ?></option>
        <?php 
         $sql3 = "select * from tbl_prison";
                $res3 = $connection->query($sql3);
                $data3 = $res3->fetch_assoc();
                if($res3->num_rows >0){
                    foreach ($data3 as $key => $prison) {
